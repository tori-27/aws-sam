...
// index.ts
var lambda_deploy_tenant_stack_exports = {};
__export(lambda_deploy_tenant_stack_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(lambda_deploy_tenant_stack_exports);
...
var cf = new import_client_cloudformation.CloudFormationClient({});
...
var TABLE_TENANT_STACK_MAPPING = "TenantStackMapping";
...
async function stackExists(stackName) {
  try {
    await cf.send(new import_client_cloudformation.DescribeStacksCommand({ StackName: stackName }));
    return true;
  } catch (err) {
...
async function createStack(stackName, templateUrl, params) {
...
async function updateStack(stackName, templateUrl, params) {
  try {
    console.log(`Updating stack: ${stackName}`);
    await cf.send(
      new import_client_cloudformation.UpdateStackCommand({
        StackName: stackName,
        TemplateURL: templateUrl,
        Capabilities: ["CAPABILITY_NAMED_IAM", "CAPABILITY_AUTO_EXPAND"],
        Parameters: params
...
function getTenantParams(tenantId) {
  return [
    {
...
async function updateTenantStackMapping(tenantId, commitId) {
  await dynamo.send(
...
async function putJobSuccess(jobId, message) {
  ...
  await codepipeline.send(
    new import_client_codepipeline.PutJobSuccessResultCommand({
      jobId
    })
  );
}
async function putJobFailure(jobId, message) {
  ...
  await codepipeline.send(
    new import_client_codepipeline.PutJobFailureResultCommand({
      jobId,
      failureDetails: {
        message,
        type: "JobFailed"
...
async function continueJobLater(jobId, message) {
...
async function checkStackUpdateStatus(jobId, stack) {
  const status = await getStackStatus(stack);
